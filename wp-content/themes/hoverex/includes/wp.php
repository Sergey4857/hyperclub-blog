...
if ( !function_exists( 'hoverex_get_nav_menu' ) ) {
	function hoverex_get_nav_menu($location='', $menu = '', $depth=0, $custom_walker=false) {
		static $list = array();
		$class = '';
			...
			foreach( $terms as $term ) {
				$output .= ($output ? $delimiter : '') 
						. '<a href="' . esc_url( get_term_link( $term->term_id, $taxonomy ) ) . '"'
							...
							. ' title="' . sprintf( esc_attr__( 'View all posts in %s', 'hoverex' ), esc_attr($term->name) ) . '"'
							. '>' 
							. esc_html( $term->name ) 
						. '</a>';
			}
...
if ( ! function_exists('hoverex_get_privacy_text' ) ) {
    function hoverex_get_privacy_text() {
        ...
        return apply_filters( 'hoverex_filter_privacy_text', wp_kses(
                $privacy_text
...
/* wp_kses handlers
----------------------------------------------------------------------------------------------------- */
if ( ! function_exists( 'hoverex_kses_allowed_html' ) ) {
    add_filter( 'wp_kses_allowed_html', 'hoverex_kses_allowed_html', 10, 2);
    function hoverex_kses_allowed_html($tags, $context) {
        if ( in_array( $context, array( 'hoverex_kses_content', 'trx_addons_kses_content' ) ) ) {
            $tags = array(
