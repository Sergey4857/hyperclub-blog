...
if ( ! class_exists( 'Hoverex_Theme_Updater' ) ) {
	...
	class Hoverex_Skin_Install{
		...
		public function __construct() {
            ...
            add_filter( 'trx_updater_filter_theme_purchase_key', array( $this, 'trx_updater_theme_purchase_key' ) );
        ...
        public function trx_updater_theme_purchase_key() {
            return get_option('hoverex_theme_code_activation');
        }
			...
			if (is_admin() && isset($current_page) && $current_page === 'hoverex_about') {
				...
				if ( !empty( $nonce ) ) {
					...
					if ( !wp_verify_nonce( $nonce, admin_url('admin-ajax.php') ) ) {
						...
						$upgrade_url = sprintf(
							hoverex_get_protocol() . '://upgrade.themerex.net/upgrade.php?key=%1$s&src=%2$s&theme_slug=%3$s&theme_name=%4$s&action=check',
							urlencode( $code ),
		...
		public function install_skin() {
			$theme_slug = empty( $_POST['theme_skin'] ) ? false :  sanitize_text_field($_POST['theme_skin']);
			...
			$theme_info  = hoverex_get_theme_info();
			$upgrade_url = sprintf(
				hoverex_get_protocol() . '://upgrade.themerex.net/upgrade.php?key=%1$s&src=%2$s&theme_slug=%3$s&theme_name=%4$s&skin=%5$s&action=install_skin',
		...
		public function remote_skin_version( $skin ) {
			$theme_info  = hoverex_get_theme_info();
			$upgrade_url = sprintf(
				hoverex_get_protocol() . '://upgrade.themerex.net/upgrade.php?theme_slug=%1$s&skin=%2$s&action=info_skin',
				urlencode( $theme_info['theme_slug'] ),
				urlencode( $skin )
			);
			$result = hoverex_fgc( $upgrade_url );
