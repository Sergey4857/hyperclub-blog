...
$HOVEREX_STORAGE = array(
	...
	'required_plugins' => array_merge(
		...
		array(
			...
			'elementor'						=> esc_html__('Elementor', 'hoverex'),
			'timeline-widget-addon-for-elementor'	=> esc_html__('Elementor Timeline Widget Addon', 'hoverex'),
			'woocommerce'					=> esc_html__('Woocommerce', 'hoverex'),
			'elegro-payment'				=> esc_html__('Elegro Payment', 'hoverex'),
			'gdpr-framework'				=> esc_html__('The GDPR Framework', 'hoverex'),			// to connect plugin support files  
			'wp-gdpr-compliance'			=> esc_html__('Cookie Information', 'hoverex'),			// to connect plugin support files
		),
		...
		//-----------------------------------------------------
			: array(
					...
					'm-chart-highcharts-library'  => esc_html__( 'M chart highcharts library', 'hoverex' ),
					...
					'trx_updater'                 => esc_html__( 'ThemeREX Updater', 'hoverex' ),
	...
	'theme_demo_url'      => '//hoverex.themerex.net',
	'theme_doc_url'       => '//hoverex.themerex.net/doc',
	'theme_demofiles_url' => '//demofiles.themerex.net/hoverex',
	'theme_download_url'  => '//1.envato.market/2ry0m0',
...
if ( !function_exists( 'hoverex_importer_set_options' ) ) {
	...
	function hoverex_importer_set_options($options=array()) {
		if (is_array($options)) {
			...
			$options['debug'] = false;
			// Prepare demo data
			$options['demo_url'] = esc_url( hoverex_get_protocol() . ':' . hoverex_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
			...
			$options['files']['default']['domain_dev'] = esc_url( hoverex_get_protocol() . ':' . hoverex_storage_get( 'theme_demo_url' ) );	// Developers domain
			$options['files']['default']['domain_demo']= esc_url( hoverex_get_protocol() . ':' . hoverex_storage_get( 'theme_demo_url' ) ); // Demo-site domain
			...
			$options['banners'] = array(
				array(
					...
					'content' => wp_kses_data(__("Create your front page right in the WordPress Customizer. There's no need in WPBakery Page Builder, or any other builder. Simply enable/disable sections, fill them out with content, and customize to your liking.", 'hoverex')),
				...
				array(
					...
					'content' => wp_kses_data(__('Use Layouts Builder to create and customize header and footer styles for your website. With a flexible page builder interface and custom shortcodes, you can create as many header and footer layouts as you want with ease.', 'hoverex')),
				...
				array(
					...
					'content' => wp_kses_data(__('Need more details? Please check our full online documentation for detailed information on how to use Hoverex.', 'hoverex')),
				...
				array(
					...
					'content' => wp_kses_data(__('No time for reading documentation? Check out our video tutorials and learn how to customize Hoverex in detail.', 'hoverex')),
				...
				array(
					...
					'title' => esc_html__('Website Customization', 'hoverex'),
					'content' => wp_kses_data(__("Need a website fast? Order our custom service, and we'll build a website based on this theme for a very fair price. We can also implement additional functionality such as website translation, setting up WPML, and much more.", 'hoverex')),
	...
	function hoverex_create_theme_options() {
		...
		hoverex_storage_set('options', array(
            ...
            'privacy_text' => array(
                ...
                "std"   => wp_kses_data( __( 'I agree that my submitted data is being collected and stored.', 'hoverex') ),
                "type"  => "text"
            ),
			...
			'header_style' => array(
				...
				"desc" => wp_kses_data( __("Select custom header from Layouts Builder", 'hoverex') ),
				"override" => array(
					'mode' => 'page,cpt_team,cpt_services,cpt_dishes,cpt_competitions,cpt_rounds,cpt_matches,cpt_cars,cpt_properties,cpt_courses,cpt_portfolio',
					'section' => esc_html__('Header', 'hoverex')
				),
			...
			'footer_style' => array(
				"title" => esc_html__('Select custom layout', 'hoverex'),
				"desc" => wp_kses_data( __("Select custom footer from Layouts Builder", 'hoverex') ),
				"override" => array(
					'mode' => 'page,cpt_team,cpt_services,cpt_dishes,cpt_competitions,cpt_rounds,cpt_matches,cpt_cars,cpt_properties,cpt_courses,cpt_portfolio',
					'section' => esc_html__('Footer', 'hoverex')
				),
		...
		foreach ($theme_fonts as $tag=>$v) {
			$fonts["{$tag}_section"] = array(
				...
				"desc" => !empty($v['description']) 
								...
								: wp_kses( sprintf(__('Font settings of the "%s" tag.', 'hoverex'), $tag), 'hoverex_kses_content' ),
				"type" => "section",
				);
