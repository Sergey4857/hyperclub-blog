	...
	function hoverex_ajax_get_posts_callback() {
		...
		$response = array(
			...
			'data' => hoverex_show_portfolio_posts(array(
							'cat' => intval(wp_unslash(hoverex_get_value_gpc('cat'))),
							'parent_cat' => intval(wp_unslash(hoverex_get_value_gpc('parent_cat'))),
							'page' => intval(wp_unslash(hoverex_get_value_gpc('page'))),
							'post_type' => trim(wp_unslash(hoverex_get_value_gpc('post_type'))),
							'taxonomy' => trim(wp_unslash(hoverex_get_value_gpc('taxonomy'))),
							'blog_style' => trim(wp_unslash(hoverex_get_value_gpc('blog_style'))),
							'echo' => false
							)
						)
		);
    ...
    function hoverex_single_comments_field( $args ) {
        ...
        if ( 'text' == $args['field_type'] ) {
            ...
            $html .= '<input id="' . esc_attr( $args['field_name'] ) . '" name="' . esc_attr( $args['field_name'] ) . '" type="checkbox" value="' . esc_attr( $args['field_value'] ) . '"' . ( $args['field_req'] ? ' aria-required="true"' : '' ) . ' />'
                    . ' <label for="' . esc_attr( $args['field_name'] ) . '" class="' . esc_attr( $args['field_req'] ? 'required' : 'optional' ) . '">' . wp_kses( $args['field_title'], 'hoverex_kses_content' ) . '</label>';
        } else {
        ...
        if ( 'default' != $args['form_style'] ) {
            $html .= '<span class="sc_form_field_hover">'
                        ...
                        . '<span class="sc_form_field_content" data-content="' . esc_attr( $args['field_title'] ) . '">' . wp_kses( $args['field_title'], 'hoverex_kses_content' ) . '</span>'
                    . '</span>';
        }
        $html .= '</span></div>';
        return $html;
