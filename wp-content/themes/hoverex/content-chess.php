...
?><article id="post-<?php the_ID(); ?>" 
	...
	if ($hoverex_columns == 1 && shortcode_exists('trx_sc_anchor')) {
		echo do_shortcode('[trx_sc_anchor id="post_'.esc_attr(get_the_ID()).'" title="'.the_title_attribute( array( 'echo' => false ) ).'" icon="'.esc_attr(hoverex_get_post_icon()).'"]');
	}
	...
	?><div class="post_inner"><div class="post_inner_content"><?php 
		...
		<div class="post_content entry-content">
			...
			if ( $hoverex_show_learn_more ) {
				?><p><a class="more-link" href="<?php the_permalink(); ?>"><?php esc_html_e('Read more', 'hoverex'); ?></a></p><?php
			}
			?>
		</div><!-- .entry-content -->
