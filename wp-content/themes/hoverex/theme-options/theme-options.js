...
jQuery(document).ready(function() {
    ...
    // Submit form
    jQuery('.hoverex_options_button_submit').on('click', function(e) {
...
function hoverex_options_check_dependencies(cont) {
    if ( typeof hoverex_dependencies == 'undefined' || HOVEREX_STORAGE['check_dependencies_now'] ) {
        return;
    }
    HOVEREX_STORAGE['check_dependencies_now'] = true;
    cont.find( '.hoverex_options_item_field,.hoverex_options_group[data-param]' ).each( function() {
        ...
        if (depend) {
            ...
            for (i in depend) {
                ...
                if ( name.charAt( 0 ) == '#' || name.charAt( 0 ) == '.' || name.slice( 0, 8 ) == '@editor/' ) {
                    ...
                    if ( fld && fld.length > 0 ) {
                        ...
                        if ( panel.length === 0 ) {
                            if ( ! fld.hasClass('hoverex_inited') ) {
                            ...
                            if ( ! panel.hasClass('hoverex_inited') ) {
                                panel.addClass('hoverex_inited').on('change', fld, function () {
                                    jQuery('.hoverex_options .hoverex_options_section').each( function () {
                                        hoverex_options_check_dependencies(jQuery(this));
                                    } );
                                } );
                        ...
                        if ( typeof wp == 'object' && typeof wp.data == 'object' && typeof wp.data.select( 'core/editor' ) == 'object' ) {
                            if ( typeof HOVEREX_STORAGE['editor_props'] == 'undefined' ) {
                                HOVEREX_STORAGE['editor_props'] = {};
                            }
                            if ( typeof HOVEREX_STORAGE['editor_props'][ prop_check ] == 'undefined' ) {
                                var prop_val = wp.data.select( 'core/editor' ).getEditedPostAttribute( prop_check );
                                if ( prop_val !== undefined ) {
                                    HOVEREX_STORAGE['editor_props'][ prop_check ] = prop_val;
                                }
                            }
                            val = typeof HOVEREX_STORAGE['editor_props'][ prop_check ] != 'undefined' ? HOVEREX_STORAGE['editor_props'][ prop_check ] : '';
                            var $body = jQuery( 'body' );
                            if ( ! $body.hasClass( 'hoverex_editor_props_listener_inited' ) ) {
                                $body.addClass( 'hoverex_editor_props_listener_inited' );
                                // Call a check_dependencies() on a page template is changed
                                wp.data.subscribe( function() {
                                    var prop_val = wp.data.select( 'core/editor' ).getEditedPostAttribute( prop_check );
                                    if ( prop_val !== undefined && ( typeof HOVEREX_STORAGE['editor_props'][ prop_check ] == 'undefined' || prop_val != HOVEREX_STORAGE['editor_props'][ prop_check ] ) ) {
                                        HOVEREX_STORAGE['editor_props'][ prop_check ] = prop_val;
                                        jQuery('.hoverex_options .hoverex_options_section').each( function () {
                                            hoverex_options_check_dependencies( jQuery( this ) );
                                        } );
                ...
                if ( val !== undefined || ( fld && fld.length > 0 ) ) {
                    ...
                    if ( val == 'inherit' ) {
                        ...
                        var parent = ctrl,
                            tag;
                        if ( ! parent.hasClass('hoverex_options_group') ) {
                            parent = parent.parents('.hoverex_options_item');
                        }
                        var lock = parent.find( '.hoverex_options_inherit_lock' );
                        if ( lock.length ) {
                            if ( ! parent.hasClass( 'hoverex_options_inherit_on' ) ) {
                                lock.trigger( 'click' );
                            }
            ...
            if ( ! ctrl.hasClass('hoverex_options_group') ) {
                ctrl = ctrl.parents('.hoverex_options_item');
            }
            var section = ctrl.parents('.hoverex_tabs_section'),
                tab = jQuery( '[aria-labelledby="' + section.attr('aria-labelledby') + '"]' );
            if (((dep_cnt > 0 || dep_all === 0) && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
                ctrl.slideDown().removeClass( 'hoverex_options_no_use' );
                if ( section.find('>.hoverex_options_item:not(.hoverex_options_item_info),>.hoverex_options_group[data-param]').length != section.find('.hoverex_options_no_use').length ) {
                    if ( tab.hasClass( 'hoverex_options_item_hidden' ) ) {
                        tab.removeClass('hoverex_options_item_hidden');
                    }
                }
            } else {
                ctrl.slideUp().addClass( 'hoverex_options_no_use' );
                if ( section.find('>.hoverex_options_item:not(.hoverex_options_item_info),>.hoverex_options_group[data-param]').length == section.find('.hoverex_options_no_use').length ) {
                    if ( ! tab.hasClass( 'hoverex_options_item_hidden' ) ) {
                        tab.addClass('hoverex_options_item_hidden');
                        if ( tab.hasClass('ui-state-active') ) {
                            tab.parents('.hoverex_tabs').find(' > ul > li:not(.hoverex_options_item_hidden)').eq(0).find('> a').trigger('click');
                        }
                    }
                }
            }
    ...
    HOVEREX_STORAGE['check_dependencies_now'] = false;
}
...
function hoverex_options_get_field_value(fld, num) {
    var item = fld.parents( '.hoverex_options_item' );
    ...
    if ( item.length && item.hasClass( 'hoverex_options_inherit_on' ) ) {
