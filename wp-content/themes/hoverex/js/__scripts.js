...
jQuery(document).ready(function() {
	...
	function hoverex_resize_video(cont) {
		...
		cont.find( 'video' ).each( function() {
			...
			var video     = $self.addClass( 'hoverex_resize' ).eq( 0 );
		...
		cont.find( '.video_frame iframe,iframe' ).each( function() {
			...
			if ( $self.hasClass( 'trx_addons_resize' )
				...
				|| $self.addClass( 'hoverex_resize' ).parents( 'div:hidden,section:hidden,article:hidden' ).length > 0
			) {
				return;
			}
	...
	function hoverex_init_post_formats(e, cont) {
		...
		hoverex_add_select_container( cont );
	...
	// Wrap select with .select_container
	function hoverex_add_select_container( cont ) {
		cont.find( 'select:not(.esg-sorting-select):not([class*="trx_addons_attrib_"]):not([size]):not(.country_to_state):not(.state_select)' ).each(
			function() {
				var $self = jQuery( this );
	...
	jQuery( document ).on( 'ajaxComplete', function(e) {
		...
		setTimeout( function() {
			hoverex_add_select_container( jQuery( 'body' ) );
		}, 100 );
	} );
	
	// Init media elements
	//--------------------------------------------
	HOVEREX_STORAGE['mejs_attempts'] = 0;
	function hoverex_init_media_elements(cont) {
		...
		if (HOVEREX_STORAGE['use_mediaelements'] && cont.find( media_selector ).length > 0) {
			if ( window.mejs ) {
			...
			} else if ( HOVEREX_STORAGE['mejs_attempts']++ < 5 ) {
	...
	//image effect
	jQuery('a[rel="magnific"]:not(.esgbox)').each(function(){
...
(function() {
	...
	jQuery( document ).on(
		'action.ready_hoverex', function() {

			// Change display mode
			jQuery( '.woocommerce,.woocommerce-page' ).on(
				'click', '.hoverex_shop_mode_buttons a', function(e) {
					console.log(1);
					var mode = jQuery( this ).hasClass( 'woocommerce_thumbs' ) ? 'thumbs' : 'list';
					hoverex_set_cookie( 'hoverex_shop_mode', mode, 365 );
			...
			if (jQuery( '.woocommerce div.quantity .q_inc,.woocommerce-page div.quantity .q_inc' ).length == 0) {
				...
				jQuery( '.woocommerce div.quantity,.woocommerce-page div.quantity' ).on(
					'click', '>span', function(e) {
						hoverex_woocomerce_inc_dec_click( jQuery( this ) );
						e.preventDefault();
						return false;
					}
				);
			...
			jQuery( document ).on(
				'action.prepare_stretch_width', function() {
					if (HOVEREX_STORAGE['stretch_tabs_area'] > 0) {
            ...
            function hoverex_woocomerce_inc_dec_click(button) {
			...
			if (jQuery( 'table.cart' ).length > 0) {
				hoverex_woocommerce_update_cart( 'init' );
			}
			...
			jQuery( window ).resize(
				function() {
					"use strict";
					if (jQuery( 'table.cart' ).length > 0) {
						hoverex_woocommerce_update_cart( 'resize' );
					}
				}
			);
			...
			jQuery( document.body ).on(
				'updated_wc_div', function() {
					"use strict";
					if (jQuery( 'table.cart' ).length > 0) {
						hoverex_woocommerce_update_cart( 'update' );
					}
				}
			);

			// Update cart display
			function hoverex_woocommerce_update_cart(status){
				"use strict";
				setTimeout(
					function() {
						var w = window.innerWidth;
						...
						if (HOVEREX_STORAGE['mobile_layout_width'] < w && status == 'resize' && jQuery( 'table.cart .mobile_cell' ).length > 0) {
							var tbl = jQuery( 'table.cart' );
							if ( tbl.length > 0 ) {
