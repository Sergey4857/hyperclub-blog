...
if (!function_exists('hoverex_trx_addons_theme_setup9')) {
	...
	function hoverex_trx_addons_theme_setup9() {
		if (hoverex_exists_trx_addons()) {
			...
			add_filter( 'hoverex_filter_post_content', 'hoverex_trx_addons_filter_post_content' );
			if (is_admin()) {
	...
	function hoverex_is_layouts_available() {
		return hoverex_exists_trx_addons() 
										&& (
                                            ...
                                            ||
                                            function_exists('hoverex_exists_elementor') && hoverex_exists_elementor()
											);
	}
}
...
if ( ! function_exists( 'hoverex_trx_addons_filter_post_content' ) ) {
	
	function hoverex_trx_addons_filter_post_content( $content ) {
...
if ( ! function_exists( 'hoverex_get_custom_layout_id' ) ) {
	function hoverex_get_custom_layout_id( $type, $layout_style = '' ) {
		$layout_id = 0;
		if ( empty( $layout_style ) ) {
			$layout_style = hoverex_get_theme_option( "{$type}_style" );
		}
		if ( strpos( $layout_style, "{$type}-custom-" ) !== false ) {
			...
			if ( 0 == (int) $layout_id ) {
				...
				$layout_id = apply_filters( 'hoverex_filter_get_translated_layout', $layout_id );
			}
		}
		return $layout_id;
	}
...
if ( ! function_exists( 'hoverex_get_custom_header_id' ) ) {
	function hoverex_get_custom_header_id() {
		static $layout_id = -1;
		if ( -1 == $layout_id && hoverex_get_theme_option( 'header_type' ) == 'custom' ) {
			$layout_id = hoverex_get_custom_layout_id( 'header' );
		}
		return $layout_id;
	}
}

if ( ! function_exists( 'hoverex_get_custom_footer_id' ) ) {
	function hoverex_get_custom_footer_id() {
		static $layout_id = -1;
		if ( -1 == $layout_id && hoverex_get_theme_option( 'footer_type' ) == 'custom' ) {
			$layout_id = hoverex_get_custom_layout_id( 'footer' );
		}
		return $layout_id;
	}
}
...
if ( ! function_exists( 'hoverex_trx_addons_default_layouts' ) ) {
	...
	function hoverex_trx_addons_default_layouts( $default_layouts = array() ) {
		if ( hoverex_storage_isset( 'trx_addons_default_layouts' ) ) {
			...
			include_once hoverex_get_file_dir( 'theme-specific/trx_addons-layouts.php' );
			if ( ! isset( $layouts ) || ! is_array( $layouts ) ) {
				$layouts = array();
			} else if ( function_exists( 'trx_addons_url_replace' ) ) {
				// Replace demo-site urls with current site url
				$layouts = trx_addons_url_replace( hoverex_storage_get( 'theme_demo_url' ), get_home_url(), $layouts );
			}
	...
	function hoverex_trx_addons_layouts_styles() {
		if (!hoverex_exists_trx_addons()) {
			if (hoverex_get_file_dir('plugins/trx_addons/layouts/layouts.css')!='') {
				wp_enqueue_style( 'hoverex-trx-addons-layouts',  hoverex_get_file_url('plugins/trx_addons/layouts/layouts.css'), array(), null );
			}
			if (hoverex_get_file_dir('plugins/trx_addons/layouts/layouts.responsive.css')!='') {
				wp_enqueue_style( 'hoverex-trx-addons-layouts-responsive',  hoverex_get_file_url('plugins/trx_addons/layouts/layouts.responsive.css'), array(), null );
			}
		}
	}
}
	...
	function hoverex_trx_addons_frontend_scripts() {
		if (hoverex_exists_trx_addons()) {
			if (hoverex_is_on(hoverex_get_theme_option('debug_mode')) && hoverex_get_file_dir('plugins/trx_addons/trx_addons.js')!='')
				wp_enqueue_script( 'hoverex-trx-addons', hoverex_get_file_url('plugins/trx_addons/trx_addons.js'), array('jquery'), null, true );
		}
	}
}
